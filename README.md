...
Il Flow Editor permette di aggiungere liberamente nodi da una palette visuale, configurarli usando i nomi di componenti e sorgenti, collegare porte success/error, uscite nominate dello switch o i percorsi “ogni elemento/completato” del loop e osservare ogni valore. Il ciclo impone sia un limite visuale di elementi sia un tetto globale di esecuzione. Stato applicativo, attese e debounce, composizione di testo e liste, API HTTP/HTTPS, CRUD, filtro, ordinamento e KPI usano lo stesso runtime. La configurazione API espone metodo e corpo senza accettare protocolli eseguibili né salvare password nel nodo. Quando i nodi comuni non bastano, **Funzione avanzata** crea una trasformazione tipizzata e protetta, la prova con casi input/risultato atteso, la esegue nel flow e la esporta come modulo TypeScript leggibile senza `eval`.
