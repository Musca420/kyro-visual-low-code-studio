...
Il Flow Editor permette di aggiungere liberamente nodi da una palette ricercabile, organizzata in **Interazioni**, **Dati e API** e **Avanzato** con apertura progressiva. Configurazione e collegamenti usano nomi comprensibili: success/error, uscite nominate dello switch o percorsi “ogni elemento/completato” del loop. Il ciclo impone sia un limite visuale di elementi sia un tetto globale di esecuzione. Stato applicativo, attese e debounce, composizione di testo e liste, API HTTP/HTTPS, CRUD, filtro, ordinamento e KPI usano lo stesso runtime. Quando i nodi comuni non bastano, **Funzione avanzata** crea una trasformazione tipizzata e protetta, la prova con casi input/risultato atteso, la esegue nel flow e la esporta come modulo TypeScript leggibile senza `eval`.
