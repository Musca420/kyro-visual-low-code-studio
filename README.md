...
Il Flow Editor permette di aggiungere liberamente nodi da una palette visuale, configurarli usando i nomi di componenti e sorgenti, collegare porte success/error e osservare il valore che attraversa stato applicativo, attese e debounce, composizione di testo e liste, API HTTP/HTTPS, caricamento, filtro, ordinamento e KPI. La configurazione API espone metodo e corpo senza accettare protocolli eseguibili né salvare password nel nodo. Le condizioni diramano senza trasformare un esito falso in errore; le porte mostrano i tipi e rifiutano collegamenti incompatibili. Quando i nodi comuni non bastano, **Funzione avanzata** crea una trasformazione tipizzata e protetta, la prova con casi input/risultato atteso, la esegue nel flow e la esporta come modulo TypeScript leggibile senza `eval`. Rinomina, eliminazione e collegamenti persistono nello stesso progetto versionato.
