...
try {
  ...
  await page.getByRole("button", { name: /^Flow/ }).click(); await page.getByLabel("Active flow").selectOption({ label: "Open Booking details from deep link" }); await page.waitForTimeout(1800);
  await page.screenshot({ path: resolve("artifacts", "nexusfield", "46-mobile-native-suite.png"), fullPage: true });
  console.log(JSON.stringify({ flows: await page.getByLabel("Active flow").locator("option").allTextContents() }, null, 2)); await page.waitForTimeout(5000);
