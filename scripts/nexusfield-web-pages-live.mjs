...
try {
  ...
  for (const [name, route] of screens) {
    if (await page.locator(".page-list button").filter({ hasText: name }).count()) continue;
  ...
  await page.screenshot({ path: resolve("artifacts", "nexusfield", "27-web-pages.png"), fullPage: true });
  ...
  await page.screenshot({ path: resolve("artifacts", "nexusfield", "failure-web-pages.png"), fullPage: true });
  throw error;
