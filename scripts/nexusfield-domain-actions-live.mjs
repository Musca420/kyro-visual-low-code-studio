...
async function apply(pageName, componentName, prompt) {
  ...
  await page.getByRole("region", { name: `Actions for ${componentName}` }).getByRole("button", { name: "Ask Codex", exact: true }).click();
...
try {
  ...
  await apply("Home", "Pending activity", "Record a sandbox payment data record, only customer or admin may run it, with success and error paths.");
