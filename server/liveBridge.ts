...
import { codexExecArguments, codexFailure, codexFinalMessage, codexThreadId, codexUsage, codexUsedDisallowedShell } from "./codexInvocation";
import { buildAgentContext } from "./agentContext";
import { operationCatalog, operationNameSet, operationPrompt } from "../src/agentRegistry";
...
import { JobStore, type JobAuditEvent } from "./jobStore";
import { assertLocalRequest, authorizeTask, defaultSecurityBudget, redactSecrets, safeEnvironment, SecurityPolicy, type SecurityBudget } from "./security";
...
const kyroMcpPath = fileURLToPath(new URL("./kyroMcp.mjs", import.meta.url));
const agentApplySchemaPath = fileURLToPath(new URL("./agent-apply.schema.json", import.meta.url));
      ...
      server.middlewares.use(async (request, response, next) => {
        try {
          ...
          if (url.pathname === "/api/live/status" && request.method === "GET") {
            const projectId = url.searchParams.get("projectId"), clientId = url.searchParams.get("clientId");
            const state = projectId
              ? (clientId ? projects.get(`${projectId}:${clientId}`) : undefined) ?? projects.get(projectId)
              : latest;
            return reply(
              response,
              state ? 200 : 503,
          ...
          if (url.pathname === "/api/agent/operations" && request.method === "GET") {
            const activeJob = authorizedAgentJob(request.headers.authorization);
            if (!activeJob) return reply(response, 401, { error: "An active Kyro Job authorization is required" });
            return reply(response, 200, { version: 1, operations: operationCatalog });
          }
          ...
          if (
            ...
            const trustedPackages = new Map<string, string>([
              ...
              ["@capacitor/app", "^8.0.0"], ["@capacitor/status-bar", "^8.0.0"],
...
Use the Kyro context pack provided below. Do not run commands or read project files. Use kyro_get_context only when the supplied indexed slice is insufficient. Before returning a plan, call kyro_plan exactly once with every operation, capability and effect ID you intend to use; copy argument shapes only from its operationContracts. Use kyro_verify or kyro_build_preflight only for an explicit verification/readiness question.
...
Available operations: ${operationPrompt}
In the plan output every operation uses {"type":"...","pageId":"page ID or null","args":{...}}. kyro_plan.operationContracts is authoritative for arguments, effects, permissions, platforms and limitations; never invent a field or type.
Return explicit typed requirements and targetPlatforms. Every planned operation needs a matching {"kind":"operation","id":"operation_name"} requirement. Operation IDs must come from kyro_plan.registeredOperations and effect IDs must come from kyro_plan.registeredEffects; never invent either. A genuinely missing capability ID may remain only when capabilityProposal supplies its exact reviewed draft. Prefer a reusable visual flow. External code or packages require explicit confirmation and a separate reviewed extension plan: never invent or install them silently.
Return exactly these top-level fields: summary, skill, requirements, targetPlatforms, operations, checks, confirmations, alreadySatisfied, capabilityProposal. Use checks for observable acceptance criteria; confirmations is always an array of plain strings.
Every plan must return alreadySatisfied and capabilityProposal. Set alreadySatisfied=true only when the indexed graph proves the requested outcome already exists; then return operations=[] and capabilityProposal=null. Otherwise set it to false. When registered graph operations fully express a required change, use capabilityProposal=null. When they cannot, do not add a fake intent operation: return operations=[] and copy the exact capabilityProposal shape from kyro_plan.capabilityProposalContract. A proposal is global, generalized for future projects and remains inactive.
...
kyro_apply_verified_transaction returns the actual rendered Preview image. Inspect that image and the VerificationReport against the original request and every approved check. Never treat the operation list or your own expected outcome as evidence. Return status="completed" only when the returned Preview visibly supports the requested result and Verification passed. Otherwise return status="needs_revision" with the concrete mismatch; never claim an unobserved interaction or visual change.
...
`;
            ...
            const execArgs = codexExecArguments(mode, threadId, mode === "plan" ? "" : agentApplySchemaPath);
            ...
            spawnedAgent.on("close", async (code) => {
              ...
              if (job.status === "completed" && mode === "plan") {
                const plan = parseAgentPlan(finalMessage);
                if (!plan) {
                  job.status = "error";
                  job.errors += "\nCodex did not return a valid typed Kyro plan. Retry is available; no project change was applied.";
                } else {
                  const resolution = plan.alreadySatisfied ? { status: "supported", explanation: "The indexed graph already proves the requested outcome", missingRequirements: [], blockedRequirements: [] } : resolveCapability({ requirements: plan.requirements, targetPlatforms: plan.targetPlatforms }, current);
                  const proposedCapabilityGap = proposalCoversCapabilityGap(resolution as ReturnType<typeof resolveCapability>, plan.capabilityProposal);
                  if (["unknown", "partial", "unsupported"].includes(resolution.status) && !proposedCapabilityGap) {
                    job.status = "error";
                    job.errors += `\nKyro cannot prove this plan: ${resolution.explanation}. Missing: ${resolution.missingRequirements.map((item) => item.id).join(", ") || "none"}. Blocked: ${resolution.blockedRequirements.map((item) => item.id).join(", ") || "none"}.`;
                  }
                }
              }
