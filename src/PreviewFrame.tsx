...
const feInput=(event,binding)=>{const configured=binding.inputId&&document.querySelector('[data-component="'+CSS.escape(binding.inputId)+'"]');if(configured&&'value'in configured)return configured.value;const target=event.target;if(event.type==='submit'&&target instanceof HTMLFormElement)return Object.fromEntries(new FormData(target));if(target instanceof HTMLInputElement&&target.type==='file')return target.files?.[0];if(event.detail?.kyroGesture)return event.detail;return target&&'value'in target?target.value:''};
