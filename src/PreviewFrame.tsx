...
export function PreviewFrame({
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      if (!preview) {
        onCaptureError?.("Preview is unavailable");
        return;
      }
      void captureElement(preview)
        .then((result) => onCapture?.(result))
        .catch((problem) =>
          onCaptureError?.(
            problem instanceof Error ? problem.message : String(problem),
          ),
