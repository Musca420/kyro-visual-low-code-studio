...
type Props = {
  ...
  onRunFlow?: (flowId: string, input: unknown) => Promise<{ notification?: string; level?: string; navigate?: string; modalId?: string; ui?: { componentId: string; operation: string; value: string } }>;
...
export function PreviewFrame({
  ...
  const srcDoc = useMemo(() => {
    const components =
      experience === "landing"
        ? project.pages.flatMap((candidate) => candidate.components)
        : (page?.components ?? []);
    const markup =
      experience === "landing"
