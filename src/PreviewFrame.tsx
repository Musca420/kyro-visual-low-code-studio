...
function flowTriggerScript(pages: Project["pages"], flows: Project["flows"], experience: unknown, interactive: boolean, activePageId?: string) {
  ...
  const automatic = flows.flatMap((flow) => flow.nodes
    .filter((node) => node.type === "event" && ["pageLoad", "timer", "pageVisible", "pageHidden", "online", "offline", "deviceShake"].includes(node.config.trigger) && (!node.config.pageId || node.config.pageId === activePageId))
