...
export function buildBottomNavigation(project: Project, activePath?: string) {
  ...
  const configuredPaths = new Set(navigation.items.map((item) => item.path));
  const remaining = project.pages
    .filter((page) => !page.path.includes(":") && !configuredPaths.has(page.path))
    .map((page) => ({ label: page.name, path: page.path }));
  const overflow = remaining.length ? [...navigation.items.slice(4), ...remaining] : [];
  const primary = overflow.length ? navigation.items.slice(0, 4) : navigation.items;
  const button = (item: { label: string; path: string }) =>
  ...
  const more = overflow.length
    ? `<details class="app-nav-more"><summary${overflow.some((item) => item.path === activePath) ? ' aria-current="page"' : ""}>More</summary><div role="menu">${overflow.map(button).join("")}</div></details>`
    : "";
  return `<style>[data-flow-status]{bottom:calc(76px + env(safe-area-inset-bottom))!important}.app-nav-more{position:relative;display:grid}.app-nav-more summary{display:grid;place-items:center;min-height:44px;padding:8px;border-radius:10px;font-size:12px;font-weight:700;cursor:pointer;list-style:none}.app-nav-more summary::-webkit-details-marker{display:none}.app-nav-more summary[aria-current=page]{background:#6d5dfc;color:#fff}.app-nav-more>div{position:absolute;right:0;bottom:calc(100% + 8px);z-index:30;display:grid;min-width:180px;gap:4px;padding:8px;border:1px solid #cfd4df;border-radius:12px;background:#171a1f;color:#fff;box-shadow:0 16px 40px #0005}.app-nav-more>div button{text-align:left}</style><nav class="app-bottom-nav" aria-label="Primary navigation">${primary
    .map(
      button,
    )
    .join("")}${more}</nav>`;
...
async function rasterizePreview(html: string, width: number, height: number) {
  ...
  try {
    await new Promise<void>((resolve, reject) => {
      copy.onload = () => resolve();
      copy.onerror = () =>
        reject(new Error("Could not prepare the preview capture"));
      copy.srcdoc = html.replace(/<script[\s\S]*?<\/script>/gi, "");
      document.body.append(copy);
    });
    const root = copy.contentDocument?.documentElement;
    if (!root) throw new Error("Preview DOM is unavailable");
...
function flowTriggerScript(pages: Project["pages"], flows: Project["flows"], experience: unknown, interactive: boolean, activePageId?: string) {
  ...
  const automatic = flows.flatMap((flow) => flow.nodes
    .filter((node) => node.type === "event" && ["pageLoad", "timer", "pageVisible", "pageHidden", "online", "offline", "deviceShake", "deepLink"].includes(node.config.trigger) && (!node.config.pageId || node.config.pageId === activePageId))
...
function dataVisualizationScript(components: EditorComponent[]) {
  const bindings = components
    .filter((component) => component.binding && (["chart", "calendar"].includes(component.type) || component.props.metric))
    .map((component) => ({ componentId: component.id, sourceId: component.binding!.sourceId, type: component.type, metric: String(component.props.metric || ""), suffix: String(component.props.metricSuffix || "") }));
  if (!bindings.length) return "";
  const safeBindings = JSON.stringify(bindings).replaceAll("<", "\\u003c");
  return `;(()=>{const bindings=${safeBindings},cache={};const dateOf=(item)=>String(item.dueDate||item.date||item.lastCompletedAt||'').slice(0,10),done=(item)=>Boolean(item.completed||item.completedToday)||/complet/i.test(String(item.status||'')),recordsFor=(message,binding)=>message.recordsBySource?.[binding.sourceId]||cache[binding.sourceId]||message.records||[],metric=(items,name)=>name==='completed'?items.filter(done).length:name==='active'?items.filter((item)=>!done(item)).length:name==='maxStreak'?Math.max(0,...items.map((item)=>Number(item.bestStreak||item.currentStreak)||0)):items.length;const render=(binding,items)=>{cache[binding.sourceId]=items;const root=document.querySelector('[data-component="'+CSS.escape(binding.componentId)+'"]');if(!root)return;if(binding.type==='chart'){const today=new Date(),days=Array.from({length:7},(_,index)=>{const day=new Date(today);day.setDate(today.getDate()-6+index);return day.toISOString().slice(0,10)}),values=days.map((day)=>items.filter((item)=>dateOf(item)===day&&(binding.metric!=='completed'||done(item))).length),max=Math.max(1,...values);root.querySelectorAll('rect').forEach((bar,index)=>{const height=Math.max(4,Math.round((values[index]||0)/max*106));bar.setAttribute('y',String(124-height));bar.setAttribute('height',String(height))});const svg=root.querySelector('svg');svg?.setAttribute('aria-label','Last seven days: '+values.join(', '));const caption=root.querySelector('figcaption');if(caption)caption.textContent=(caption.dataset.label||caption.textContent||'Chart')+' · '+values.reduce((sum,value)=>sum+value,0)+' total';if(caption&&!caption.dataset.label)caption.dataset.label=(caption.textContent||'').split(' · ')[0]}else if(binding.type==='calendar'){const input=root.querySelector('input[type=date]'),list=root.querySelector('ul');if(!input||!list)return;if(!input.value)input.value=new Date().toISOString().slice(0,10);const shown=items.filter((item)=>dateOf(item)===input.value);list.replaceChildren(...(shown.length?shown.map((item)=>{const row=document.createElement('li');row.textContent=String(item.title||item.name||item.text||'Item')+(item.time?' · '+item.time:'');return row}):[Object.assign(document.createElement('li'),{textContent:'No items on this date'})]));if(!input.dataset.bound){input.dataset.bound='true';input.addEventListener('change',()=>render(binding,cache[binding.sourceId]||[]))}}else if(binding.metric){const target=root.querySelector('strong')||root;target.textContent=String(metric(items,binding.metric))+(binding.suffix?' '+binding.suffix:'')}};addEventListener('message',(event)=>{const message=event.data;if(message?.channel!=='frontend-editor-host')return;bindings.forEach((binding)=>render(binding,recordsFor(message,binding)))})})();`;
}

function signatureScript(components: EditorComponent[]) {
  if (!components.some((component) => component.type === "signature")) return "";
  return `;document.querySelectorAll('[data-signature-canvas]').forEach((canvas)=>{const input=canvas.parentElement.querySelector('input[type=hidden]'),clear=canvas.parentElement.querySelector('[data-clear-signature]'),context=canvas.getContext('2d'),resize=()=>{const box=canvas.getBoundingClientRect(),ratio=Math.max(1,devicePixelRatio||1),width=Math.max(1,Math.round(box.width*ratio)),height=Math.max(1,Math.round(140*ratio));if(canvas.width===width&&canvas.height===height)return;canvas.width=width;canvas.height=height;context.setTransform(ratio,0,0,ratio,0,0);context.lineWidth=2.5;context.lineCap='round';context.strokeStyle=getComputedStyle(canvas).color},point=(event)=>{const box=canvas.getBoundingClientRect();return{x:event.clientX-box.left,y:event.clientY-box.top}};let drawing=false;requestAnimationFrame(resize);new ResizeObserver(resize).observe(canvas);canvas.addEventListener('pointerdown',(event)=>{resize();drawing=true;const value=point(event);context.beginPath();context.moveTo(value.x,value.y);try{canvas.setPointerCapture(event.pointerId)}catch{}});canvas.addEventListener('pointermove',(event)=>{if(!drawing)return;const value=point(event);context.lineTo(value.x,value.y);context.stroke()});const finish=()=>{if(!drawing)return;drawing=false;input.value=canvas.toDataURL('image/png');input.dispatchEvent(new Event('change',{bubbles:true}))};canvas.addEventListener('pointerup',finish);canvas.addEventListener('pointercancel',finish);clear.addEventListener('click',()=>{context.clearRect(0,0,canvas.width,canvas.height);input.value='';input.dispatchEvent(new Event('change',{bubbles:true}))})});`;
}
...
function landingMarkup(components: EditorComponent[]) {
  ...
  const base = `<nav ${attr(nav)} aria-label="Main navigation"><a class="logo" href="#top">${label(nav)}</a><div>${links.map((link) => `<a href="#${escapeHtml(link.toLowerCase())}">${escapeHtml(link)}</a>`).join("")}</div><button class="mobile-menu" aria-label="Open menu" aria-expanded="false">☰</button></nav><main id="top" class="landing"><section class="hero"><div class="hero-copy"><p class="kicker">PROJECT CLARITY, DELIVERED</p><h1 ${attr(title)}>${label(title)}</h1><p ${attr(copy)}>${label(copy)}</p><div class="hero-actions"><button ${attr(primary)} data-landing-action="features">${label(primary)}</button><button ${attr(secondary)} data-landing-action="notify">${label(secondary)}</button></div></div><div ${attr(visual)} class="hero-visual" role="img" aria-label="${label(visual)}"><div class="visual-window"><span></span><span></span><span></span><article><i></i><b></b></article><article><i></i><b></b></article><article><i></i><b></b></article></div></div></section><section id="features" class="features"><p class="kicker">BUILT FOR MOMENTUM</p><h2 ${attr(featureTitle)}>${label(featureTitle)}</h2><div class="feature-grid">${features.map((feature) => `<article ${attr(feature)}><span>${escapeHtml(feature.props.icon)}</span><h3>${label(feature)}</h3><p>${escapeHtml(feature.props.description)}</p><button class="feature-more" data-open-modal>Learn more <span aria-hidden="true">→</span></button></article>`).join("")}</div></section><section ${attr(cta)} class="landing-cta"><div><h2>${label(cta)}</h2><p>${escapeHtml(cta?.props.description)}</p></div><button data-page-target="contact">Start free today</button></section></main><footer ${attr(footer)}>${label(footer)}</footer><div class="modal-backdrop" hidden><section ${attr(modal)} role="dialog" aria-modal="true" aria-labelledby="landing-modal-title"><button class="modal-close" aria-label="Close dialog">×</button><h2 id="landing-modal-title">${label(modal)}</h2><p>${escapeHtml(modal?.props.description)}</p><button data-page-target="contact">Let's talk</button></section></div><div ${attr(toast)} class="toast" role="status" hidden>${label(toast)}</div>`;
  const richPages = `<section class="rich-page pricing-page" id="pricing" hidden><p class="kicker">PRICING</p><h1 ${attr(pricingTitle)}>${label(pricingTitle)}</h1><label class="plan-search">Search plans<input ${attr(pricingSearch)} type="search" placeholder="${escapeHtml(pricingSearch?.props.placeholder)}"></label><div class="pricing-grid">${plans.map((plan, index) => `<article ${attr(plan)} data-plan="${label(plan).toLowerCase()}"><span>${index === 1 ? "Most popular" : "Plan"}</span><h2>${label(plan)}</h2><strong>€${[19, 49, 99][index] ?? 29}<small>/month</small></strong><p>${escapeHtml(plan.props.description)}</p><button data-open-modal>Explore plan</button></article>`).join("")}</div><p class="plan-empty" hidden>No plan matches your search.</p></section><section class="rich-page contact-page" id="contact" hidden><div><p class="kicker">LET'S BUILD</p><h1 ${attr(contactTitle)}>${label(contactTitle)}</h1><p>Tell us what you want to build. The request is saved to the source connected to this flow.</p></div><form ${attr(contactForm)} novalidate><label>Name<input ${attr(contactName)} name="name" required minlength="2" placeholder="${escapeHtml(contactName?.props.placeholder)}"></label><label>Email<input name="email" type="email" required placeholder="name@company.com"></label><label>Message<textarea name="message" required minlength="8" placeholder="Goal, timeline, and audience"></textarea></label><div class="contact-error" role="alert"></div><button ${attr(contactSubmit)} type="submit">${label(contactSubmit)}</button></form><section ${attr(contactList)} class="contact-history" aria-label="Submitted requests"><h2>${label(contactList)}</h2><div class="loading" role="status">Loading…</div><div class="empty" hidden>No requests submitted.</div><ul></ul></section></section>`;
  const validation = `<script>document.addEventListener('invalid',(event)=>{const form=event.target.closest('.contact-page form');if(form)form.querySelector('.contact-error').textContent='Complete name, email, and message with valid values.'},true)</script>`;
...
export function buildExperienceAssets(
  ...
  return experience === "landing"
    ...
    : {
        ...
        css: `${dashboardCss()}@media(max-width:620px){.dash-logo{font-size:20px}.dashboard-shell aside nav a{font-size:14px;text-align:left}.user-card div{display:grid}}`,
        script: dashboardScript(true) + dashboardValidationScript(),
      };
}

export function previewExperienceForPage(experience: unknown, components: EditorComponent[]) {
  if (experience === "landing") return "landing" as const;
  if (experience === "dashboard" && components.some((component) => component.props.slot === "sidebar" || component.props.slot === "dashboard-title")) return "dashboard" as const;
  return undefined;
}
...
export function PreviewFrame({
  ...
  const runtime = useMemo(() => compileRuntimeProgram(project), [project]);
  const { pages, flows, dataSources, theme } = runtime;
  const page = pages.find((candidate) => candidate.id === pageId);
  const sourceId = page?.components.find((component) => component.binding?.state === "data")?.binding?.sourceId ?? dataSources[0]?.id;
  const boundSourceIds = useMemo(() => [...new Set((page?.components ?? []).flatMap((component) => component.binding?.sourceId ? [component.binding.sourceId] : []))], [page]);
  ...
  const experience = runtime.state.experience;
  const srcDoc = useMemo(() => {
    const pageExperience = previewExperienceForPage(experience, page?.components ?? []);
    const components =
      pageExperience === "landing"
        ? pages.flatMap((candidate) => candidate.components)
        : (page?.components ?? []);
    const markup =
      pageExperience === "landing"
        ? landingMarkup(components)
        : pageExperience === "dashboard"
          ? dashboardMarkup(components)
          : `<main>${runtime.pages.find((candidate) => candidate.id === pageId)?.markup ?? ""}</main>`;
    const navigation = buildBottomNavigation(project, page?.path);
    const css =
      pageExperience === "landing"
        ? landingCss()
        : pageExperience === "dashboard"
          ? `${dashboardCss()}@media(max-width:620px){.dash-logo{font-size:20px}.dashboard-shell aside nav a{font-size:14px;text-align:left}.user-card div{display:grid}}`
    ...
    const hasBoundList = components.some((component) => component.type === "list" && component.binding?.state === "data");
    const behavior =
      pageExperience === "landing"
        ? landingScript(interactive) + landingValidationScript()
        : pageExperience === "dashboard"
          ? dashboardScript(interactive) + dashboardValidationScript()
          : (hasBoundList ? todoListRenderer(interactive && !hasGraphBindings) + stableRecordActionsScript(tracksStreak) + (supportsStructuredEdit ? recordUpdateToastScript() + recordEditScript() + recordEditClickGuardScript() : "") : "");
    const readySignal = behavior.includes("send('READY')")
      ? ""
      : "send('READY');";
    ...
    const observability = `;const feLogText=(value)=>{try{return typeof value==='string'?value:JSON.stringify(value)}catch{return String(value)}};['log','info','warn','error'].forEach((level)=>{const original=console[level].bind(console);console[level]=(...values)=>{original(...values);send('RUNTIME_LOG',{level:level==='error'?'error':'info',message:values.map(feLogText).join(' ')})}});addEventListener('error',(event)=>send('RUNTIME_LOG',{level:'error',message:event.message||'Runtime error'}));addEventListener('unhandledrejection',(event)=>send('RUNTIME_LOG',{level:'error',message:'Unhandled promise: '+feLogText(event.reason)}));`;
    return `<!doctype html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><style>:root{font-family:Inter,ui-sans-serif,system-ui,sans-serif;color:#172033;background:#f7f8fc}html[data-theme=dark]{color-scheme:dark;color:#f3f4f6;background:#0f1115}html[data-theme=dark] body{color:#f3f4f6!important;background:#0f1115!important;background-image:none!important}html[data-theme=dark] input,html[data-theme=dark] textarea,html[data-theme=dark] select{color:#f3f4f6;background:#1d2229;border-color:#4b5563}*{box-sizing:border-box}[hidden]{display:none!important}body{margin:0}button,input,textarea,select{font:inherit}button{cursor:pointer;border:0;border-radius:10px;padding:11px 14px;background:#6d5dfc;color:#fff;font-weight:700}button:focus-visible,input:focus-visible,textarea:focus-visible,select:focus-visible,a:focus-visible{outline:3px solid #8b7fff;outline-offset:3px}input,textarea,select{width:100%;border:1px solid #cfd4df;border-radius:9px;padding:10px;background:#fff}label{display:grid;gap:5px;font-size:12px;font-weight:700}.signature-pad canvas{width:100%;height:140px;touch-action:none;border:1px solid #cfd4df;border-radius:10px;background:#fff;color:#172033}.choice-control{display:flex;align-items:center;gap:10px;min-height:44px;cursor:pointer}.choice-control input{width:20px;height:20px;min-width:20px;margin:0;padding:0;accent-color:#6d5dfc}.choice-control span{line-height:1.35}ul{list-style:none;padding:0}.record-row{display:grid;gap:6px;padding:14px 0;border-bottom:1px solid #ffffff20}.record-row p{margin:0;opacity:.78}.record-row div{display:flex;gap:8px;align-items:center;flex-wrap:wrap}.record-row time{margin-left:auto;font-size:12px;opacity:.72}.chip{display:inline-flex;padding:4px 8px;border-radius:999px;background:#ffffff18;font-size:11px}.preview-container,.generated-container{display:grid;gap:12px}.preview-grid,.generated-grid{grid-template-columns:repeat(auto-fit,minmax(180px,1fr))}.app-bottom-nav{position:sticky;z-index:20;bottom:0;display:grid;grid-auto-flow:column;grid-auto-columns:1fr;gap:4px;padding:8px max(8px,env(safe-area-inset-right)) calc(8px + env(safe-area-inset-bottom)) max(8px,env(safe-area-inset-left));border-top:1px solid #cfd4df;background:${theme.tokens.pageBackground ?? "#ffffff"};backdrop-filter:blur(16px)}.app-bottom-nav button{min-height:44px;padding:8px;background:transparent;color:inherit;font-size:12px}.app-bottom-nav button[aria-current=page]{background:#6d5dfc;color:#fff}@keyframes fe-fade{from{opacity:0}to{opacity:1}}@keyframes fe-rise{from{opacity:0;transform:translateY(18px)}to{opacity:1;transform:none}}@keyframes fe-pulse{50%{transform:scale(1.04)}}@keyframes fe-float{50%{transform:translateY(-8px)}}.error{color:#b42318}.empty,.loading{color:#697386;padding:12px 0}${css}${components.map((component) => styleFor(component, breakpoint)).join("\n")}body{background:${theme.tokens.pageBackground ?? "#ffffff"};background-image:${theme.tokens.pageBackgroundImage ?? "none"};background-size:cover;background-position:center}</style></head><body>${markup}${navigation}<script>const send=(type,payload={})=>parent.postMessage({channel:'frontend-editor-preview',type,...payload},'*');${observability}${capture}${navigationBehavior}${themeBehavior}${behavior}${readySignal}${signatureScript(components)}${dataVisualizationScript(components)}${flowTriggerScript(pages, flows, pageExperience, interactive, page?.id)}</script></body></html>`;
  }, [project, runtime, pageId, page, pages, flows, theme, breakpoint, interactive, experience, tracksStreak, supportsStructuredEdit]);

  useEffect(() => {
    const refreshSources = async () => Object.fromEntries(await Promise.all(boundSourceIds.map(async (id) => [id, await onRefresh(id)] as const)));
    const listener = async (event: MessageEvent) => {
      ...
      if (event.data.type === "READY") {
        ready.current = true;
        const recordsBySource = await refreshSources();
        const records = sourceId ? recordsBySource[sourceId] ?? await onRefresh(sourceId) : [];
        frame.current?.contentWindow?.postMessage(
          { channel: "frontend-editor-host", records, recordsBySource, error },
          "*",
        );
        if (captureRequest)
          frame.current?.contentWindow?.postMessage(
      ...
      if (event.data.type === "RUN_FLOW" && onRunFlow) {
        try {
          ...
          const recordsBySource = await refreshSources();
          frame.current?.contentWindow?.postMessage({ channel: "frontend-editor-host", action: "flow", records: sourceId ? recordsBySource[sourceId] ?? [] : [], recordsBySource, ...outcome }, "*");
        } catch (problem) {
      ...
      if (event.data.type === "RECORD_ACTION" && onRecordAction) {
        try {
          ...
          frame.current?.contentWindow?.postMessage({ channel: "frontend-editor-host", records, recordsBySource: await refreshSources(), action }, "*");
        } catch (problem) {
          const message = problem instanceof Error ? problem.message : String(problem);
  ...
  useEffect(() => {
    ...
    const timeout = setTimeout(
      () => onCaptureError?.("Preview capture timed out"),
      8_000,
    );
    return () => {
      clearTimeout(resend);
    ...
    // The request id is the transaction boundary. Callback identity changes must
    // not repeat a completed capture.
  ...
  return (
    <iframe
      ...
      data-runtime-adapter={previewRuntimeAdapter.target}
    />
  );
}
