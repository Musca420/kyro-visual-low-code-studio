...
export function App() {
  ...
  if (active) return <Editor initial={active} onClose={() => { setActive(undefined); void refresh() }} />
  return <Dashboard loading={loading} projects={projects} onOpen={async (id) => { const project = await getProject(id); if (project) setActive(project) }} onRefresh={refresh} />
}
...
function Editor({ initial, onClose }: { initial: Project; onClose: () => void }) {
  ...
  return <div className="app-shell">
    ...
    <nav className="workspace-tabs" aria-label="Aree di lavoro">{([['design', 'Design'], ['flow', 'Flow'], ['data', 'Dati'], ['preview', 'Preview'], ['plugins', 'Plugin']] as [WorkspaceTab, string][]).map(([value, label]) => <button key={value} className={tab === value ? 'active' : ''} onClick={() => setTab(value)}>{label}{value === 'flow' && project.flows.length > 0 && <span className="badge">{project.flows.length}</span>}</button>)}</nav>
    ...
    {tab === 'design' && <div className="editor-grid">
      <aside className="left-panel"><PanelTitle eyebrow="Struttura" title="Pagine" /><div className="page-list">{project.pages.map((page) => <button className={page.id === pageId ? 'active' : ''} key={page.id} onClick={() => { setPageId(page.id); setSelected([]) }}><span>▱</span>{page.name}<small>{page.path}</small></button>)}<button className="dashed" onClick={addPage}>＋ Aggiungi pagina</button></div><PanelTitle eyebrow="Elementi" title="Palette" /><div className="palette">{componentTypes.map((type) => <button key={type} draggable onDragStart={(event) => event.dataTransfer.setData('application/frontend-component', type)} onClick={() => addComponent(type)}><span>{icon(type)}</span>{type}</button>)}</div></aside>
...
function DesignComponent({ component, breakpoint, selected, onSelect, onMove }: { component: EditorComponent; breakpoint: Breakpoint; selected: boolean; onSelect: (multi: boolean) => void; onMove: (direction: number) => void }) {
