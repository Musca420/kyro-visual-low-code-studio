...
function Dashboard({ loading, projects, onOpen, onRefresh }: { loading: boolean; projects: Project[]; onOpen: (id: string) => void; onRefresh: () => Promise<void> }) {
  ...
  const importFile = async (file?: File) => {
    ...
    try {
      ...
      setError(`Import non riuscito: ${problem instanceof Error ? problem.message : String(problem)}`)
    }
  }
  ...
  return (
    <main className="dashboard">
      <header className="hero">
        ...
        <p>Crea interfaccia, dati e comportamento nello stesso spazio. Il progetto resta tuo, esportabile e leggibile.</p>
      </header>
      <section className="create-card" aria-labelledby="create-title">
        ...
        <div className="template-grid">
          ...
          <button className="template featured" onClick={() => create('landing')}>
            <span>↗</span>
            <strong>Landing page</strong>
            <small>Hero, feature, CTA e footer</small>
          </button>
          <button className="template featured" onClick={() => create('dashboard')}>
            <span>▦</span>
            <strong>Project dashboard</strong>
            <small>KPI, ricerca, filtri e CRUD</small>
          </button>
        </div>
            ...
            {projects.map((project) => (
              <article className="project-card" key={project.id}>
                <button className="project-open" onClick={() => onOpen(project.id)}>
                  ...
                  <small>
                    Formato v{project.formatVersion} · {project.pages.length} pagine
                  </small>
                </button>
                <div className="project-actions">
                  ...
                  <button
                    className="danger"
                    onClick={async () => {
                      if (confirm(`Eliminare definitivamente “${project.name}”?`)) {
...
function Editor({ initial, onClose }: { initial: Project; onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    const layouts = Object.fromEntries(
      currentPage.components.map((component) => {
        ...
        return [component.id, box ? { x: box.x, y: box.y, width: box.width, height: box.height } : null]
      }),
    )
    const state = {
  ...
  const askCodex = (action: string) => {
    ...
    const context: CodexContext = {
      ...
      treePath: [currentPage.name, ...componentPath(currentPage.components, component.id).map((item) => item.name)],
      ...
      dataSources: project.dataSources,
      flows: project.flows.filter((item) => Object.values(component.events).includes(item.id)),
      nearbyComponents: currentPage.components
        .filter((item) => item.id !== component.id)
        .slice(0, 8)
        .map(({ id, name, type }) => ({ id, name, type })),
      generatedFiles: ['src/main.ts', 'src/style.css', 'project.frontend-editor.json'],
      errors: [],
    }
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      void saveProject(project)
        ...
        .catch((error) => setSaveState(`Errore salvataggio: ${error instanceof Error ? error.message : String(error)}`))
    }, 450)
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      void captureElement(canvas)
        ...
        .catch((error) => finishBridgeCommand(captureCommand.id, undefined, error))
    }, 100)
    return () => clearTimeout(timer)
  ...
  useEffect(() => {
    const timer = setInterval(async () => {
      try {
        const commands = (await fetch(`/api/live/commands?projectId=${project.id}`).then((response) => response.json())) as {
          id: string
          tool: string
        ...
        for (const command of commands) {
          ...
          try {
            if (command.tool === 'capture_canvas' || command.tool === 'capture_preview') {
  ...
  const patchPage = (update: (components: EditorComponent[]) => EditorComponent[]) =>
    change((value) => ({
      ...value,
      pages: value.pages.map((page) => (page.id === pageId ? { ...page, components: update(page.components) } : page)),
    }))
  ...
  const removeSelected = () => {
    change((value) => {
      ...
      if (page) selected.forEach((id) => descendantIds(page.components, id).forEach((childId) => removedComponents.add(childId)))
      const removedFlows = new Set(value.flows.filter((item) => item.nodes.some((node) => node.config.componentId && removedComponents.has(node.config.componentId))).map((item) => item.id))
      return {
        ...value,
        ...
        pages: value.pages.map((page) => ({
          ...
          components: page.components
            ...
            .map((component) => ({
              ...component,
              events: Object.fromEntries(Object.entries(component.events).filter(([, flowId]) => !removedFlows.has(flowId))),
            })),
        })),
      }
    })
  ...
  const reorder = (component: EditorComponent, direction: number) => {
    if (!currentPage) return
    const siblings = currentPage.components.filter((item) => item.parentId === component.parentId)
    const index = siblings.findIndex((item) => item.id === component.id) + direction
  ...
  return (
    <div className="app-shell" data-project-id={project.id}>
      <header className="topbar">
        <button className="brand-button" onClick={onClose} aria-label="Chiudi progetto e torna alla dashboard">
          <span>FE</span>
        </button>
        <div className="project-title">
          <input aria-label="Nome progetto" value={project.name} onChange={(event) => change({ ...project, name: event.target.value })} />
          <span>{saveState}</span>
        </div>
        <div className="top-actions">
          <button className="icon-button" onClick={undo} disabled={!history.length} aria-label="Annulla">
            ↶
          </button>
          <button className="icon-button" onClick={redo} disabled={!future.length} aria-label="Ripristina">
            ↷
          </button>
          ...
          <button
            onClick={() =>
              void downloadGeneratedApp(project)
      ...
      <nav className="workspace-tabs" aria-label="Aree di lavoro">
        {(
          [
            ['design', 'Design', 'Disegna pagine e componenti senza scrivere codice.'],
            ['flow', 'Flow', 'Definisci cosa accade quando l’utente interagisce.'],
          ...
          <button key={value} data-help={help} className={tab === value ? 'active' : ''} onClick={() => setTab(value)}>
            {label}
            {value === 'flow' && project.flows.length > 0 && <span className="badge">{project.flows.length}</span>}
          </button>
        ))}
      </nav>
        ...
        <div className="editor-grid">
          <aside className="left-panel">
            ...
            <div className="page-list">
              ...
              <button data-help="Crea una nuova schermata vuota nel progetto." className="dashed" onClick={addPage}>
                ＋ Aggiungi pagina
              </button>
            </div>
            ...
            <div className="palette">
              {componentTypes.map((type) => (
                <button data-help={componentHelp[type]} key={type} draggable onDragStart={(event) => event.dataTransfer.setData('application/frontend-component', type)} onClick={() => addComponent(type)}>
                  <span>{icon(type)}</span>
                  {type}
                </button>
              ))}
          ...
          <section className="canvas-area">
            <div className="canvas-toolbar">
              <div className="segmented" aria-label="Breakpoint">
                {BREAKPOINTS.map((value) => (
                  <button key={value} className={breakpoint === value ? 'active' : ''} onClick={() => setBreakpoint(value)}>
                    {value}
                  </button>
                ))}
              </div>
              <div className="zoom">
                <button onClick={() => setZoom(Math.max(0.5, zoom - 0.1))} aria-label="Riduci zoom">
                  −
                </button>
                <output>{Math.round(zoom * 100)}%</output>
                <button onClick={() => setZoom(Math.min(1.5, zoom + 0.1))} aria-label="Aumenta zoom">
                  ＋
                </button>
              </div>
            </div>
            <div className="canvas-scroll">
              <div
                ...
                onDrop={(event) => {
                  const type = event.dataTransfer.getData('application/frontend-component') as EditorComponent['type']
                ...
                {!currentPage ? (
                  ...
                  branches.map((branch) => (
                    <DesignBranch
                      ...
                      selected={selected}
                      onSelect={(id, multi) => setSelected((items) => (multi ? (items.includes(id) ? items.filter((item) => item !== id) : [...items, id]) : [id]))}
          ...
          <aside className="right-panel">
            <PanelTitle eyebrow="Ispezione" title={selected.length > 1 ? `${selected.length} elementi` : (activeComponent?.name ?? 'Proprietà')} />
            {activeComponent ? (
              <Properties
                ...
                onReparent={(parentId) => reparent(activeComponent.id, parentId)}
                onWrap={() => wrap(activeComponent.id)}
                onDuplicate={() => {
                  const copies = selected
                    .map((id) => currentPage!.components.find((item) => item.id === id))
                    .filter(Boolean)
                    .map((item) => ({
                      ...item!,
                      id: crypto.randomUUID(),
            ...
            <ol className="layers" role="tree">
              {branches.map((branch) => (
                <LayerBranch key={branch.component.id} branch={branch} level={1} selected={selected} onSelect={(id) => setSelected([id])} />
              ))}
            </ol>
          </aside>
        </div>
        ...
        <main className="wide-workspace">
          <div className="section-heading">
            <div>
              ...
              <h1>Flow editor</h1>
              <p>Collega eventi e operazioni. I percorsi success ed error sono distinti e validati.</p>
              {project.flows.length > 0 && (
                <label>
                  Flow attivo
                  <select aria-label="Flow attivo" value={flow?.id} onChange={(event) => setFlowId(event.target.value)}>
            ...
            <button onClick={createFlow}>{project.state.experience === 'landing' ? 'Crea interazioni landing' : project.state.experience === 'dashboard' ? 'Crea flow dashboard' : 'Crea flow dati'}</button>
          </div>
          <Suspense fallback={<div className="flow-canvas">Caricamento editor flow…</div>}>
            <FlowEditor
              flow={flow}
              onChange={(updated) =>
                change({
        ...
        <main className="wide-workspace">
          <div className="section-heading">
            <div>
              ...
              <p>IndexedDB conserva i record sul dispositivo. Nessun segreto richiesto o salvato.</p>
            </div>
          </div>
      ...
      {contextMenu && (
        <div className="component-menu" role="menu" aria-label={`Azioni per ${contextMenu.component.name}`} style={{ left: contextMenu.x, top: contextMenu.y }}>
          {['Chiedi a Codex', 'Crea comportamento', 'Modifica comportamento', 'Collega dati', 'Correggi problema', 'Migliora componente', 'Spiega elemento'].map((action) => (
            <button role="menuitem" key={action} onClick={() => askCodex(action)}>
              {action === 'Chiedi a Codex' ? '⌘' : '›'}
              <span>{action}</span>
            </button>
          ))}
          <button role="menuitem" className="menu-cancel" onClick={() => setContextMenu(undefined)}>
            Chiudi menu
          </button>
        </div>
      )}
...
function GuideBar({ project, tab, onOpen }: { project: Project; tab: WorkspaceTab; onOpen: (tab: WorkspaceTab) => void }) {
  ...
  const steps: {
    ...
    {
      done: project.dataSources.length > 0 || project.state.experience === 'landing',
      tab: 'data',
  ...
  return (
    <aside className="guide-strip" aria-label="Percorso guidato">
      ...
      <span>{next.help}</span>
      <button data-help={`Vai in ${next.tab} per: ${next.label}.`} onClick={() => onOpen(next.tab)}>
        {tab === next.tab ? 'Sei qui' : 'Portami lì'} <span aria-hidden="true">→</span>
      </button>
      <details>
        <summary data-help="Mostra tutti i passi e quelli già completati.">Percorso</summary>
        <ol>
          {steps.map((step) => (
            <li className={step.done ? 'done' : step === next ? 'current' : ''} key={step.label}>
...
function HelpOverlay({ children }: { children: React.ReactNode }) {
  ...
  const show = (target: EventTarget | null, x?: number, y?: number) => {
    ...
    if (text)
      setHelp({
        text: element.dataset.help || `${text}: passa qui per usare questo controllo.`,
        x: x ?? element.getBoundingClientRect().left,
        y: y ?? element.getBoundingClientRect().bottom,
      })
  }
  return (
    <div className="help-surface" onPointerOver={(event) => show(event.target, event.clientX, event.clientY)} onPointerMove={(event) => show(event.target, event.clientX, event.clientY)} onPointerLeave={() => setHelp(undefined)} onFocusCapture={(event) => show(event.target)} onBlurCapture={() => setHelp(undefined)}>
      {children}
      {help && (
...
type LiveComponentBranch = EditorComponent & { children: LiveComponentBranch[] }

const serializeBranch = ({ component, children }: ComponentBranch): LiveComponentBranch => ({
  ...component,
...
function DesignBranch({ branch, breakpoint, selected, onSelect, onMove, onContextMenu, onAdd }: { branch: ComponentBranch; breakpoint: Breakpoint; selected: string[]; onSelect: (id: string, multi: boolean) => void; onMove: (component: EditorComponent, direction: number) => void; onContextMenu: (component: EditorComponent, bounds: CodexContext['bounds'], point: { x: number; y: number }) => void; onAdd: (type: EditorComponent['type'], parentId?: string) => void }) {
  ...
  return (
    <DesignComponent component={component} breakpoint={breakpoint} selected={selected.includes(component.id)} onSelect={(multi) => onSelect(component.id, multi)} onMove={(direction) => onMove(component, direction)} onContextMenu={(bounds, point) => onContextMenu(component, bounds, point)} onDrop={canContain(component) ? (type) => onAdd(type, component.id) : undefined}>
      {children.length > 0 ? children.map((child) => <DesignBranch key={child.component.id} branch={child} breakpoint={breakpoint} selected={selected} onSelect={onSelect} onMove={onMove} onContextMenu={onContextMenu} onAdd={onAdd} />) : undefined}
    </DesignComponent>
  )
}

function LayerBranch({ branch, level, selected, onSelect }: { branch: ComponentBranch; level: number; selected: string[]; onSelect: (id: string) => void }) {
  return (
    <li role="treeitem" aria-level={level} aria-expanded={branch.children.length ? true : undefined}>
      <button style={{ paddingLeft: `${8 + (level - 1) * 16}px` }} className={selected.includes(branch.component.id) ? 'active' : ''} onClick={() => onSelect(branch.component.id)}>
      ...
      {branch.children.length > 0 && (
        <ol role="group">
          {branch.children.map((child) => (
            <LayerBranch key={child.component.id} branch={child} level={level + 1} selected={selected} onSelect={onSelect} />
          ))}
        </ol>
      )}
    </li>
...
function DesignComponent({ component, breakpoint, selected, onSelect, onMove, onContextMenu, onDrop, children }: { component: EditorComponent; breakpoint: Breakpoint; selected: boolean; onSelect: (multi: boolean) => void; onMove: (direction: number) => void; onContextMenu: (bounds: CodexContext['bounds'], point: { x: number; y: number }) => void; onDrop?: (type: EditorComponent['type']) => void; children?: React.ReactNode }) {
  const style = {
    ...
    >
      ...
      {canContain(component) && (
        <div className={`component-children ${children ? 'has-children' : ''}`}>
          <span className="drop-hint">{children ? component.name : 'Trascina qui gli elementi'}</span>
          {children}
        </div>
      )}
      {selected && (
...
function Properties({ component, components, breakpoint, onUpdate, onReparent, onWrap, onDuplicate, onDelete }: { component: EditorComponent; components: EditorComponent[]; breakpoint: Breakpoint; onUpdate: (update: (component: EditorComponent) => EditorComponent) => void; onReparent: (parentId?: string) => void; onWrap: () => void; onDuplicate: () => void; onDelete: () => void }) {
  ...
  return (
    <div className="properties">
      <label>
        Nome
        <input value={component.name} onChange={(event) => onUpdate((item) => ({ ...item, name: event.target.value }))} />
      </label>
      <label>
        Testo / etichetta
        <input
      ...
      <label data-help="Scegli il contenitore visuale che deve racchiudere questo elemento. Pagina lo riporta al livello principale.">
        Dentro
        <select value={component.parentId ?? ''} onChange={(event) => onReparent(event.target.value || undefined)}>
      ...
      <div className="field-pair">
        <label>
          Larghezza
          <input value={style.width} onChange={(event) => setStyle('width', event.target.value)} />
        </label>
        <label>
          Altezza min.
          <input value={style.minHeight} onChange={(event) => setStyle('minHeight', event.target.value)} />
        </label>
      </div>
      <div className="field-pair">
        <label>
          Posizione X
          <input value={style.marginLeft} onChange={(event) => setStyle('marginLeft', event.target.value)} />
        </label>
        <label>
          Posizione Y
          <input value={style.marginTop} onChange={(event) => setStyle('marginTop', event.target.value)} />
        </label>
      </div>
      <div className="field-pair">
        <label>
          Testo
          <input type="color" value={style.color} onChange={(event) => setStyle('color', event.target.value)} />
        </label>
        <label>
          Sfondo
          <input type="color" value={style.background} onChange={(event) => setStyle('background', event.target.value)} />
        </label>
      </div>
      <label>
        Raggio bordi
        <input value={style.borderRadius} onChange={(event) => setStyle('borderRadius', event.target.value)} />
      </label>
      <label>
        Visibilità
        <select value={style.display} onChange={(event) => setStyle('display', event.target.value)}>
          <option value="block">Visibile</option>
          <option value="none">Nascosto</option>
        </select>
      </label>
      <div className="button-row">
        <button className="secondary" data-help="Crea un gruppo verticale attorno a questo elemento, utile per organizzare più elementi insieme." onClick={onWrap}>
          Raggruppa
        </button>
...
function LogConsole({ logs }: { logs: FlowLog[] }) {
  return (
    <section className="log-console" aria-labelledby="log-title">
      ...
      {logs.length === 0 ? (
        <p>Esegui il flow dalla preview per ispezionare input, output ed errori.</p>
      ) : (
        <ol>
          {logs.map((log, index) => (
...
function addVerticalTemplate(project: Project): Project {
  const page = {
    ...
    name: 'Attività',
    path: '/',
    components: [makeComponent('title'), makeComponent('input'), makeComponent('button'), makeComponent('list')],
  }
...
function landingFlows(project: Project): {
  ...
  return {
    ...
    pages: project.pages.map((page) => ({
      ...page,
      components: page.components.map((component) => (component.id === primary.id ? { ...component, events: { click: flows[0].id } } : component.id === secondary.id ? { ...component, events: { click: flows[1].id } } : component)),
    })),
  }
}
...
function dashboardFlows(project: Project): {
  ...
  const flows = [make('Carica progetti', bySlot('projects-table'), 'query'), make('Crea progetto', bySlot('create'), 'insert', true), make('Aggiorna progetto', bySlot('project-form'), 'update', true), make('Elimina progetto', bySlot('detail-modal'), 'delete'), make('Cerca progetti', bySlot('search'), 'filter'), make('Filtra stato', bySlot('filter'), 'filter'), make('Ordina progetti', bySlot('sort'), 'sort')]
  ...
  return {
    ...
    pages: project.pages.map((page) => ({
      ...
      components: page.components.map((component) => ({
        ...component,
        events: eventMap.has(String(component.props.slot)) ? { change: eventMap.get(String(component.props.slot))! } : component.events,
        ...(component.props.slot === 'projects-table' ? { binding: { sourceId: source.id, state: 'data' as const } } : {}),
      })),
    })),
  }
}
