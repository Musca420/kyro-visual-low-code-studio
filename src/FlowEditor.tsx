...
function FlowNode({ data }: NodeProps) {
  ...
  return <div className="flow-node" style={{ borderColor: colors[value.type] ?? '#64748b' }} onPointerDown={selectBody} onClick={selectBody}>
    ...
    {['validate', 'condition', 'switch', 'requireRole'].includes(value.type) && <Handle type="source" position={Position.Bottom} id="error" style={value.type === 'switch' ? { left: '10%' } : undefined} />}
...
export default function FlowEditor({ flow, pages, components, sources, modules, selectedNodeId, onChange, onModulesChange, onCreateModule, onNodeSelect }: { flow?: Flow; pages: Project['pages']; components: EditorComponent[]; sources: Project['dataSources']; modules: Project['codeModules']; selectedNodeId?: string; onChange: (flow: Flow) => void; onModulesChange: (modules: Project['codeModules']) => void; onCreateModule: (module: Project['codeModules'][number], nodeId: string) => void; onNodeSelect?: (nodeId: string) => void }) {
  ...
  const connect = (connection: Connection) => {
    ...
    if (!compatible(output, input)) return setConnectionError(`Collegamento non valido: ${labels[source.type]} produce ${output}, ${labels[target.type]} richiede ${input === 'writable' ? 'testo o record' : input}.`)
    setConnectionError('')
  ...
  const connectGuided = (path: string, targetId: string) => {
    ...
    if (!compatible(output, input)) return setConnectionError(`Collegamento non valido: ${labels[selected.type]} produce ${output}, ${labels[target.type]} richiede ${input === 'writable' ? 'testo o record' : input}.`)
    setConnectionError('')
  ...
  const groups: { label: string; types: FlowNode['type'][]; open?: boolean }[] = [
    { label: 'Interazioni', open: true, types: ['event', 'readInput', 'validate', 'condition', 'switch', 'delay', 'format', 'navigate', 'openModal', 'updateUI', 'notify', 'localNotification'] },
    { label: 'Accesso', types: ['requireRole', 'signOut'] },
    { label: 'Dati e API', types: ['insert', 'query', 'update', 'delete', 'filter', 'sort', 'map', 'kpi', 'http', 'file'] },
  ...
  return <div className="flow-builder">
    <aside className="flow-palette" aria-label="Aggiungi nodi al flow">
      ...
      <details className="flow-existing-nodes" open>
        <summary>Nodi nel flow <span>{flow.nodes.length}</span></summary>
        <nav aria-label="Nodi del flow">
    ...
    <button type="button" className="flow-fit-button" disabled={!flowInstance} onClick={fitAll}>Mostra tutto</button>
    <ReactFlow
      nodes={nodes}
      edges={edges}
      nodeTypes={nodeTypes}
    ...
    <aside className="flow-inspector" aria-label="Configura nodo selezionato">
      {selected ? <>
        ...
        {selected.type === 'event' && <><label>Quando deve partire<select aria-label="Tipo evento" value={selected.config.trigger ?? 'click'} onChange={(event) => setConfig('trigger', event.target.value)}><option value="click">Al click</option><option value="change">Quando cambia un campo</option><option value="submit">Quando viene inviato un form</option><option value="pageLoad">Quando si apre la pagina</option><option value="timer">A intervalli regolari</option></select></label>{!['pageLoad', 'timer'].includes(selected.config.trigger ?? 'click') && <label>Elemento<select aria-label="Elemento collegato" value={selected.config.componentId ?? ''} onChange={(event) => setConfig('componentId', event.target.value)}><option value="">Scegli elemento…</option>{components.map((component) => <option key={component.id} value={component.id}>{component.name} · {component.type}</option>)}</select></label>}{['pageLoad', 'timer'].includes(selected.config.trigger ?? 'click') && <label>Pagina<select aria-label="Pagina evento automatico" value={selected.config.pageId ?? ''} onChange={(event) => setConfig('pageId', event.target.value)}><option value="">Tutte le pagine (progetti precedenti)</option>{pages.map((page) => <option key={page.id} value={page.id}>{page.name}</option>)}</select></label>}{selected.config.trigger === 'timer' && <label>Ogni quanti millisecondi<input aria-label="Intervallo evento" type="number" min="500" max="3600000" value={selected.config.interval ?? '5000'} onChange={(event) => setConfig('interval', event.target.value)} /></label>}<small>Questo collegamento viene usato uguale in preview ed export.</small></>}
        ...
        {selected.type === 'openModal' && <label>Azione modal<select aria-label="Azione modal" value={selected.config.operation ?? 'open'} onChange={(event) => setConfig('operation', event.target.value)}><option value="open">Apri</option><option value="close">Chiudi</option></select></label>}
        ...
        {selected.type === 'query' && <><label>Cosa caricare<select aria-label="Tipo caricamento dati" value={selected.config.mode ?? 'all'} onChange={(event) => setConfig('mode', event.target.value)}><option value="all">Tutti i record</option><option value="one">Un record per ID</option></select></label>{selected.config.mode === 'one' && <><label>ID da trovare<input aria-label="ID record da caricare" value={selected.config.id ?? '{{value}}'} onChange={(event) => setConfig('id', event.target.value)} /></label><label>Campo dell’input che contiene l’ID<input aria-label="Campo ID input" value={selected.config.field ?? 'id'} onChange={(event) => setConfig('field', event.target.value)} /></label><small>Usa {'{{value}}'} per prendere l’ID dal passo precedente; se arriva un record viene letto il campo indicato.</small></>}</>}
        ...
        {selected.type === 'file' && <><label>Dimensione massima in MB<input aria-label="Dimensione massima file" type="number" min="1" max="10" value={selected.config.maxMb ?? '2'} onChange={(event) => setConfig('maxMb', event.target.value)} /></label><label>Tipi accettati<input aria-label="Tipi file accettati" placeholder="image/*,application/pdf" value={selected.config.accept ?? ''} onChange={(event) => setConfig('accept', event.target.value)} /></label><small>Il file viene convertito in un record con nome, tipo, dimensione e contenuto. Per file grandi usa un provider storage.</small></>}
        {selected.type === 'requireRole' && <><label>Ruoli ammessi<input aria-label="Ruoli ammessi" value={selected.config.roles ?? 'admin'} placeholder="admin,editor" onChange={(event) => setConfig('roles', event.target.value)} /></label><label>Ruolo nella preview<select aria-label="Ruolo simulato preview" value={selected.config.previewRole ?? 'viewer'} onChange={(event) => setConfig('previewRole', event.target.value)}><option value="admin">Amministratore</option><option value="editor">Editor</option><option value="viewer">Lettore</option></select></label><label>Messaggio quando bloccato<input aria-label="Messaggio accesso negato" value={selected.config.message ?? ''} onChange={(event) => setConfig('message', event.target.value)} /></label><small>La preview simula il ruolo scelto. Nell’export il ruolo arriva dalla sessione firmata del backend.</small></>}
        {selected.type === 'signOut' && <small>Rimuove la sessione locale e torna alla schermata di accesso nell’app esportata.</small>}
        ...
        {selected.type === 'navigate' && <><label>Destinazione<select aria-label="Tipo navigazione" value={selected.config.mode ?? 'page'} onChange={(event) => setConfig('mode', event.target.value)}><option value="page">Una pagina del progetto</option><option value="back">Torna indietro</option><option value="url">Un sito esterno</option></select></label>{selected.config.mode !== 'back' && <label>{selected.config.mode === 'url' ? 'Indirizzo completo' : 'Percorso pagina'}<input aria-label="Percorso navigazione" type={selected.config.mode === 'url' ? 'url' : 'text'} value={selected.config.path ?? '/'} placeholder={selected.config.mode === 'url' ? 'https://esempio.it' : '/pagina'} onChange={(event) => setConfig('path', event.target.value)} /></label>}</>}
        ...
        {selected.type === 'localNotification' && <><label>Titolo<input aria-label="Titolo notifica locale" value={selected.config.title ?? ''} onChange={(event) => setConfig('title', event.target.value)} /></label><label>Messaggio<input aria-label="Messaggio notifica locale" value={selected.config.body ?? ''} onChange={(event) => setConfig('body', event.target.value)} /></label><label>Ritardo in millisecondi<input aria-label="Ritardo notifica locale" type="number" min="0" max="604800000" value={selected.config.delayMs ?? '2000'} onChange={(event) => setConfig('delayMs', event.target.value)} /></label><small>In Android usa la notifica di sistema; sul web chiede il permesso del browser.</small></>}
