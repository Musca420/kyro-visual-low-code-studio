...
const colors: Record<string, string> = { event: '#7c3aed', readInput: '#2563eb', validate: '#d97706', condition: '#ea580c', getState: '#8b5cf6', setState: '#7c3aed', resetState: '#6d28d9', delay: '#f59e0b', format: '#a855f7', http: '#0ea5e9', insert: '#059669', query: '#0f766e', update: '#15803d', delete: '#b91c1c', filter: '#9333ea', sort: '#4f46e5', kpi: '#c2410c', refresh: '#0891b2', navigate: '#0369a1', openModal: '#be185d', notify: '#db2777', module: '#06b6d4', log: '#64748b' }
const labels: Record<FlowNode['type'], string> = { event: 'Evento', readInput: 'Leggi input', validate: 'Valida', condition: 'Condizione', getState: 'Leggi stato', setState: 'Salva stato', resetState: 'Azzera stato', delay: 'Attendi', format: 'Componi testo', http: 'Chiama API', insert: 'Crea record', query: 'Carica dati', update: 'Aggiorna record', delete: 'Elimina record', filter: 'Filtra', sort: 'Ordina', kpi: 'Calcola KPI', refresh: 'Aggiorna UI', navigate: 'Vai alla pagina', openModal: 'Apri modal', notify: 'Mostra notifica', module: 'Funzione avanzata', log: 'Debug' }
const defaults: Record<FlowNode['type'], Record<string, string>> = { event: {}, readInput: {}, validate: { message: 'Questo valore è obbligatorio' }, condition: { field: '', operator: 'equals', value: '' }, getState: { key: 'value' }, setState: { key: 'value' }, resetState: { key: 'value' }, delay: { ms: '300' }, format: { template: '{{value}}' }, http: { url: 'https://api.example.com/items', method: 'GET', body: '{{value}}' }, insert: {}, query: {}, update: {}, delete: {}, filter: { field: 'text', value: '' }, sort: { field: 'date', direction: 'asc' }, kpi: { operation: 'count' }, refresh: {}, navigate: { path: '/' }, openModal: {}, notify: { message: 'Operazione completata', level: 'success' }, module: {}, log: { message: 'Valore corrente' } }
type ValueType = 'unknown' | 'string' | 'record' | 'list' | 'number'
const ports: Record<FlowNode['type'], { input: ValueType; output: ValueType }> = { event: { input: 'unknown', output: 'unknown' }, readInput: { input: 'unknown', output: 'string' }, validate: { input: 'unknown', output: 'unknown' }, condition: { input: 'unknown', output: 'unknown' }, getState: { input: 'unknown', output: 'unknown' }, setState: { input: 'unknown', output: 'unknown' }, resetState: { input: 'unknown', output: 'unknown' }, delay: { input: 'unknown', output: 'unknown' }, format: { input: 'unknown', output: 'string' }, http: { input: 'unknown', output: 'unknown' }, insert: { input: 'string', output: 'record' }, query: { input: 'unknown', output: 'list' }, update: { input: 'record', output: 'record' }, delete: { input: 'record', output: 'unknown' }, filter: { input: 'list', output: 'list' }, sort: { input: 'list', output: 'list' }, kpi: { input: 'list', output: 'number' }, refresh: { input: 'unknown', output: 'unknown' }, navigate: { input: 'unknown', output: 'unknown' }, openModal: { input: 'unknown', output: 'unknown' }, notify: { input: 'unknown', output: 'unknown' }, module: { input: 'unknown', output: 'unknown' }, log: { input: 'unknown', output: 'unknown' } }
  ...
  return <div className="flow-builder">
    ...
    <aside className="flow-inspector" aria-label="Configura nodo selezionato">
      {selected ? <>
        ...
        {selected.type === 'delay' && <label>Attesa in millisecondi<input aria-label="Durata attesa" type="number" min="0" max="10000" value={selected.config.ms ?? '300'} onChange={(event) => setConfig('ms', event.target.value)} /></label>}
