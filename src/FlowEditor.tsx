...
const defaults: Record<FlowNode['type'], Record<string, string>> = { event: { trigger: 'click', interval: '5000' }, readInput: {}, validate: { field: '', rule: 'required', value: '', message: 'Questo valore è obbligatorio' }, condition: { field: '', operator: 'equals', value: '' }, switch: { field: '', cases: 'bozza,in corso,completato' }, loop: { max: '100' }, getState: { key: 'value' }, setState: { key: 'value' }, resetState: { key: 'value' }, delay: { ms: '300' }, debounce: { ms: '300' }, format: { template: '{{value}}' }, map: { field: 'text', template: '{{value}}' }, http: { url: 'https://api.example.com/items', method: 'GET', body: '{{value}}' }, file: { maxMb: '2', accept: '' }, requireRole: { roles: 'admin', previewRole: 'viewer', message: 'Non hai il permesso per questa azione' }, signOut: {}, insert: {}, query: {}, update: {}, delete: {}, filter: { field: 'text', value: '' }, sort: { field: 'date', direction: 'asc' }, kpi: { operation: 'count' }, refresh: {}, navigate: { mode: 'page', path: '/' }, openModal: {}, updateUI: { operation: 'show', value: '' }, notify: { message: 'Operazione completata', level: 'success' }, module: {}, log: { message: 'Valore corrente' } }
