...
test("Open Mode risolve un gap con un modulo verificato e non simula plugin mancanti", async ({ page, request }) => {
  ...
  const register = (args: Record<string, unknown>, revision = state.revision) => page.evaluate(async ({ projectId, pageId, revision, args }) => {
  ...
  }, { projectId, pageId: state.pageId, revision, args });
